...
model_files=glob('%s/*.json'%'home/omidard/models')

for m in model_files:
    mname = m.replace('home/omidard/models','')
    ...
    base=cobra.io.read_sbml_model('home/omidard/ref_model_dir/maylbr.xml')
    m9(base)
    ans = base.optimize()
    print('base = ',ans)
    print('><><><>',' base model has been loaded')
    model=cobra.io.load_json_model('home/omidard/models/'+mname)
    m9(model)
    ans = model.optimize()
    print('target = ',ans)
