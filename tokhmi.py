...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 26 17:54:51 2022
...
models =glob('%s/*.json'%'/home/omidard/allgems/inhouse/biomassed')
gr=[]
mid=[]
reactions_total = []
reactions_gap = []
...
all_gems.to_excel('/home/omidard/allgems/inhouse/information.xls')
print(all_gems)
...
temp_re_id=[] #list1
template = load_json_model('/home/omidard/allgems/inhouse/biomassed/'+temps_inf.id[len(temps_inf)-1])
for reaction in template.reactions:
    temp_re_id.append(reaction.id)
allmissing=[]
for i in failed:
    missing=pd.DataFrame()
    model = load_json_model('/home/omidard/allgems/inhouse/biomassed/'+i)
...
for i in range(len(all_gaps)):
    model = load_json_model('/home/omidard/allgems/inhouse/biomassed/'+all_gaps[i].columns[0])
    if model.optimize().fluxes.BIOMASS2 == 0:
        ...
        for x in all_gaps[i][all_gaps[i].columns[0]]:
            ...
            print('-----done')
        print(model.optimize().fluxes.BIOMASS2)
        cobra.io.json.save_json_model(model,'/home/omidard/allgems/inhouse/biomassed2/'+model.id)
