...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 25 23:31:23 2022

@author: omidard
"""

# second round gapfilling on server
...
#list of the files
directory = '/home/omidard/allgems'
...
for f in subdirs:
    #scanning the models
    print('==================================','\n','models scanning is on progress')
    models =glob('%s/*.json'%f+'biomassed')
    gr=[]
    mid=[]
    reactions_total = []
    reactions_gap = []
    ...
    all_gems.to_excel(f+'/inf.xls')
    print('GEMs report','\n','\n',all_gems)
    print('\n','\n','number of failed models = ',len(failed))
    
    #finding best template model
    
    print('==================','\n','searching for the best template model')
    ...
    print('template model is = ',temps_inf.id[0])
    
    
    
    
    #template based gapfilling
    print('=======================','\n','gapfilling is started')
    
    temp_re_id=[] #list1
    template = load_json_model(f+'/biomassed/'+temps_inf.id[24])
    for reaction in template.reactions:
        temp_re_id.append(reaction.id)
    allmissing=[]
    for i in failed:
        missing=pd.DataFrame()
        model = load_json_model(f+'/biomassed/'+i)
    ...
    for i in range(len(allmissing)):
        ...
        for x in allmissing[i][allmissing[i].columns[0]]:
            template = load_json_model(f+'/biomassed/'+temps_inf.id[24])
            m9(template)
            lb=template.reactions.get_by_id(x).lower_bound
            ub=template.reactions.get_by_id(x).upper_bound
            template.reactions.get_by_id(x).lower_bound = 0
            template.reactions.get_by_id(x).upper_bound = 0
            if template.optimize().fluxes.BIOMASS2 < 0.100000:
                gap.append(x)
    ...
    for i in range(len(all_gaps)):
        model = load_json_model(f+'/biomassed/'+all_gaps[i].columns[0])
        if model.optimize().fluxes.BIOMASS2 == 0:
            ...
            for x in all_gaps[i][all_gaps[i].columns[0]]:
                ...
                model.repair()
                m9(model)
                print('fba after gapfilling = ',model.optimize().fluxes.BIOMASS2)
                print(reaction2.id)
                print('-----done')
            cobra.io.json.save_json_model(model,f+'/biomassed2/'+model.id)
    
    
    
    
    #checking gapfilling results
    print('===================','\n','check gapfilled models')
    models =glob('%s/*.json'%f+'/biomassed2')
    gr=[]
    mid=[]
    reactions_total = []
    reactions_gap = []
