...
def get_file_names (directory):
    ...
    gl5=gl4.replace("'","")
    gl6=gl5.replace(".gbk","")
    with open(directory+'/targ.txt','w') as tf:
        tf.writelines(gl6)
        tf.close()   
    gl7 = []
    txtf = directory+'/targ.txt'
    with open(txtf) as f:
        for line in f:
            line = line.strip()
            if line != "":
                gl7.append(line)
                gl7.append
    gl8=pd.DataFrame({'strain':gl7,'NCBI ID':gl7, 'Pathotype':range(len(onlyfiles))})
    path = directory+'/StrainInformation.xlsx'
    StrainInformation = gl8.to_excel(path)
                

#%% retrives genome file names from a folder and returns an excell file with target genomes information
get_file_names (r'/Users/omidard/Desktop/fna')

#%%
# Load the information on the five strains we will be working with in this tutorial
StrainsOfInterest=pd.read_excel(r'/Users/omidard/Desktop/multispecies/Supplementary/StrainInformation.xlsx')
StrainsOfInterest
#%%
...
def make_blast_db(id,folder='prots',db_type='prot'):
    ...
    bashCommand =['cd /Users/omidard/opt/anaconda3/pkgs/blast-2.12.0-pl5262h78c34c6_0/bin','makeblastdb -in %s/%s.%s -dbtype %s' %(folder, id, ext, db_type)]
