...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 26 09:10:22 2022
...
for directorys in subdirs:
    print(directorys)
    base_model_dir = '/home/omidard/ref_model_dir/PAL060.json'
    orto_matrix = directorys+'/present_absence_dir/ortho_matrix.csv'
    alaki = directorys+'/biomassed'
    print(alaki)
    target_models = glob('%s/*.json'%alaki)
    print('target_models = ',target_models)
    gapfilled_model_dir = directorys+'/biomassed2/'
    for target_model_dir in target_models:
        strain_nam = target_model_dir.replace(directorys+'/biomassed/','')
        strain_name = strain_nam.replace('.json','')
        print('strain_nam = ',strain_name)
